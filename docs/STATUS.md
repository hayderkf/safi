...
- **الواجهة (Next.js، 3601):** صفحة تولّد استمارة، تعرضها كنموذج حيّ (RTL)، تعبّئها وتحفظها.
  ...
  - **لوحة الإدارة (`/admin`):** تظهر للمسؤول فقط (رابط «الإدارة» في الشريط). إدارة المستخدمين (إضافة، إسناد أدوار بمربّعات، تفعيل/تعطيل، حذف) + القوائم الساندة: يدوياً (إنشاء، إضافة عناصر بتتالٍ، حذف) **وبالذكاء** (وصف → اقتراح → **جدول مراجعة قابل للتعديل** → اعتماد وحفظ) **ومن Excel** + **تحرير العناصر فردياً** (عرض/تعديل العنوان والأب/حذف ناعم لكل عنصر، عبر `PATCH`/`DELETE /lookups/{key}/items/{value_key}`).
